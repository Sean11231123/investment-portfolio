...
The script reads official no-key ISIN sources and writes one merged Taiwan universe file:

- TWSE listed securities: `https://isin.twse.com.tw/isin/C_public.jsp?strMode=2`
- TPEx OTC securities: `https://isin.twse.com.tw/isin/C_public.jsp?strMode=4`
...
```text
public/data/macro/macro-indicators.json
```

It uses seasonally adjusted BLS series where available so month-over-month calculations are meaningful:
...
Taiwan listed stock/ETF prices are updated outside the frontend by:

```text
scripts/update_tw_prices.py
```

The script defaults to broad TWSE listed coverage by reading TWSE-priced symbols from:
...
The updater uses the TWSE OpenAPI `STOCK_DAY_AVG_ALL` endpoint. It writes quote entries for TWSE stock/ETF universe symbols, uses positive TWSE prices when available, and marks missing or malformed prices with `price: null` and `status: "unavailable"`. TPEx OTC assets in the Taiwan universe are intentionally skipped by this TWSE price updater. Missing prices are never forced to zero.

TPEx OTC stock/ETF prices are updated separately by:

```text
scripts/update_tpex_otc_prices.py
```

The script reads OTC assets from `public/data/universe/tw-assets.json`, fetches the official TPEx OpenAPI daily close endpoint:
...
```text
public/data/market/tpex-otc-prices.json
```

The TPEx OTC file is separate from TWSE listed prices so the app can report 上市 and 上櫃 coverage independently. Missing, malformed, empty, or zero close prices are written as `price: null` with `status: "unavailable"` and never become zero.
...
To refresh broad TWSE listed price coverage, run:

```bash
python scripts/update_tw_prices.py
```

To refresh TPEx OTC price coverage, run:

```bash
python scripts/update_tpex_otc_prices.py
```
...
- TWSE listed and TPEx OTC prices are generated from separate public sources and may have different availability.
...
- runs `python scripts/update_tpex_otc_prices.py`
- commits `public/data/market/tw-prices.json` and `public/data/market/tpex-otc-prices.json` only if either changed
...
- TPEx OTC Taiwan prices use a separate TPEx static close-price file and may still be unavailable for assets without valid source quotes.
