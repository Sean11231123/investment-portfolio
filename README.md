...
- Derive asset name, market, native currency, unit, and price source from the built-in registry plus static seed universe JSON when available.
...
## Dynamic Asset Universe Foundation

The built-in registry in `src/data/assetRegistry.ts` remains the core metadata source. Phase 5B adds a layered resolver that can also load small static seed universe files from:
...
- `tw-assets.json`: small Taiwan stock/ETF examples such as `0052` and `2603`
...
These files prove the dynamic universe architecture. They are not full market sync outputs yet.
