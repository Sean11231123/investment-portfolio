...
This universe is search metadata only. Searchable asset does not mean a price exists in `public/data/market/tw-prices.json`, and searchable ETF does not mean component data exists under `public/data/etf-components/`. TPEx OTC assets are searchable before OTC price coverage exists; Phase 5L-3 is the intended price-coverage follow-up. Missing OTC prices remain unavailable/null and are shown as untracked, never zero; ETFs without component JSON remain `未展開 ETF` when priced.
...
The updater uses the TWSE OpenAPI `STOCK_DAY_AVG_ALL` endpoint. It writes quote entries for TWSE stock/ETF universe symbols, uses positive TWSE prices when available, and marks missing or malformed prices with `price: null` and `status: "unavailable"`. TPEx OTC assets in the Taiwan universe are intentionally skipped by this TWSE price updater until a separate OTC price phase is implemented. Missing prices are never forced to zero.
...
- TPEx OTC prices are not generated by this TWSE price updater.
...
- TPEx OTC Taiwan assets are searchable metadata only until the separate OTC price pipeline is implemented.
