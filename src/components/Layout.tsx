  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#f1f5f9_100%)] text-slate-900">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 sm:px-6 sm:py-4 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 truncate text-lg font-semibold tracking-tight text-slate-900 sm:text-2xl">              Modular Investment Portfolio
            </h1>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 px-3 pb-[calc(env(safe-area-inset-bottom)+0.65rem)] sm:hidden">
        <div className="mx-auto max-w-md rounded-[1.75rem] border border-slate-200 bg-white/95 p-1.5 shadow-[0_18px_40px_rgba(15,23,42,0.10)] backdrop-blur-xl">
  ...
  return (
    <nav
      className={`relative grid-cols-4 gap-1 ${isMobile
        ? "grid rounded-[1.45rem] bg-slate-100 p-1"
        : "rounded-full border border-slate-200 bg-white p-1 shadow-sm"
      } ${className}`}
      aria-label="主選單"
    >
      <div
        ...
        className={`absolute left-1 top-1 h-[calc(100%-0.5rem)] transition-transform duration-300 ease-out ${isMobile
          ? "rounded-[1.15rem] bg-slate-900 shadow-sm"
          : "rounded-full bg-slate-900 shadow-sm"
          }`}
        style={{
